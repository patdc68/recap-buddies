-- Returnee contact details and selfies are booking-specific. This preserves an
-- existing renter's historical verification image and email while allowing the
-- current booking to use the address and selfie explicitly submitted for it.
alter table public."RB_PUBLIC_BOOKING_FLOW"
  add column if not exists notification_email text,
  add column if not exists returnee_selfie_img text;

alter table public."RB_RENTAL_FORM"
  add column if not exists notification_email text,
  add column if not exists returnee_selfie_img text;

comment on column public."RB_RENTAL_FORM".notification_email is
  'Booking-specific notification destination supplied by the renter.';
comment on column public."RB_RENTAL_FORM".returnee_selfie_img is
  'Private Storage path for the selfie submitted for this returnee booking.';
...
begin
  ...
  if length(v_email) > 254 or v_email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' then
    raise exception using errcode = '22023', message = 'Invalid returnee email address.';
  end if;
  if nullif(trim(coalesce(p_selfie_path, '')), '') is null then
    raise exception using errcode = '22023', message = 'A returnee selfie is required.';
  end if;
  ...
  if v_renter_id is null then
    ...
    insert into public."RB_RENTER" (
      renter_fname, renter_lname, mobile_no, email, auth_user_id,
      selfie_verification_id, selfie_verification_img
    ) values (
      initcap(v_first_name), initcap(v_last_name), v_phone::numeric, v_email,
      null, null, p_selfie_path
    ) returning id into v_renter_id;
    returnee_matched_existing := false;
    legacy_returnee := true;
  else
    update public."RB_RENTER"
    set email = v_email
  ...
  insert into public."RB_PUBLIC_BOOKING_FLOW" (
    token_hash, renter_id, renter_type, returnee_matched_existing,
    legacy_returnee, notification_email, returnee_selfie_img, ip_hash
  ) values (
    p_token_hash, v_renter_id, 'returnee', returnee_matched_existing,
    legacy_returnee, v_email, p_selfie_path, p_ip_hash
  );
  return next;
end;
$$;

revoke all on function public.rb_create_returnee_flow(text, text, text, text, text, text)
  from public, anon, authenticated;
grant execute on function public.rb_create_returnee_flow(text, text, text, text, text, text)
  to service_role;

-- Retain the old overload only for migration compatibility, but make it
-- uncallable so returnee sessions cannot bypass email/selfie verification.
revoke all on function public.rb_create_returnee_flow(text, text, text, text)
...
begin
  ...
  if v_flow.renter_type = 'returnee' and (
    nullif(trim(coalesce(v_flow.notification_email, '')), '') is null or
    nullif(trim(coalesce(v_flow.returnee_selfie_img, '')), '') is null
  ) then
    raise exception using errcode = 'P0001', message = 'This returnee session is missing verification details.';
  end if;
  ...
  insert into public."RB_RENTAL_FORM" (
    ...
    notification_email, returnee_selfie_img
  ) values (
    ...
    v_rent_price, 'submitted', v_flow.renter_type, v_flow.returnee_matched_existing,
    v_flow.legacy_returnee, v_flow.notification_email, v_flow.returnee_selfie_img
...
revoke all on function public.rb_submit_public_booking(text, uuid[], text, text, text, text, text, date, date, time, time, uuid, text, uuid, text)
  from public, anon, authenticated;
grant execute on function public.rb_submit_public_booking(text, uuid[], text, text, text, text, text, date, date, time, time, uuid, text, uuid, text)
  to service_role;
