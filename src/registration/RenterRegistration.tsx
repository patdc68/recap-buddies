...
const StepSelfie: React.FC<StepSelfieProps> = ({ errors, previews, onCapture }) => (
  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
    <Box>
      ...
      <Typography variant="body2">Upload or take a clear recent selfie so we can verify your account.</Typography>
    </Box>
    <Alert severity="info" sx={{ borderRadius: 2 }}>{SELFIE_REMINDER}</Alert>
    <CameraCapture
      label="Selfie Photo"
      facingMode="user"
  ...
  return (
    <PageLayout>
      ...
      >
        ...
        {activeStep === 5 && (
          <StepSelfie errors={errors} previews={previews} onCapture={onCapture} />
        )}
