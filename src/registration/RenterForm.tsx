...
import type { RbBranch, RbDevice, RbRenter, LocUsage } from '../service/supabaseClient';
...
const RepeatSelfieVerification: React.FC<RepeatSelfieVerificationProps> = ({ selfiePreview, onSelfieCapture, error, loading }) => (
  <Paper elevation={0} sx={{ p: { xs: 2, sm: 2.5 }, background: '#ffffff', border: '1px solid #f1e5d0', borderRadius: '24px', boxShadow: '0 8px 24px rgba(0,0,0,0.04)', color: '#111111' }}>
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5 }}>
      <Box sx={{ width: 42, height: 42, borderRadius: '14px', background: '#fff7e8', border: '1px solid #f1e5d0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><VerifiedUserIcon sx={{ color: '#C9973A' }} /></Box>
      <Box><Typography variant="h6" sx={{ color: '#111111', lineHeight: 1.1, fontWeight: 800 }}>Repeat Renter Selfie Verification</Typography><Typography variant="body2" sx={{ color: '#6b7280' }}>Capture a new clear selfie before submitting this rental.</Typography></Box>
    </Box>
    {loading && <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, py: 2 }}><CircularProgress size={18} sx={{ color: '#C9973A' }} /><Typography sx={{ color: '#374151', fontSize: '0.9rem' }}>Checking verification requirements…</Typography></Box>}
    <Alert severity="info" sx={{ mb: 2, borderRadius: '14px' }}>{SELFIE_REMINDER}</Alert>
    <CameraCapture label="Live Selfie Capture" facingMode="user" onCapture={onSelfieCapture} capturedUrl={selfiePreview} hint="Use good lighting and keep your full face visible." variant="lightVerification" />
...
const RenterForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadRepeatVerification = async () => {
      ...
      try {
        ...
        // Selfie instructions are no longer required; keep only the image requirement.
      } catch (err) {
        if (!cancelled) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        await sendRentalStatusEmail({ status: 'for-review', rental: createdRental, renter });
      } catch (emailError) {
